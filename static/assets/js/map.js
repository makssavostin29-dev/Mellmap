let places = []; // ← глобальная переменная
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showPlaceDetails(place) {
    ...
    const catMap = { cafe: 'Кафе', restaurant: 'Ресторан', bar: 'Бар' };
    document.getElementById('place-category').textContent = catMap[place.category] || place.category;
    document.getElementById('place-description').textContent = place.description || 'Описание отсутствует';

    // Отображение ссылки на заведение
    const websiteContainer = document.getElementById('place-website-container');
    ...
    if (place.breakfast_time === 'weekends_only') {
      ...
      breakfastTimeText = place.breakfast_hours || 'В определённое время';
    } else {
      breakfastTimeText = 'Не указано';
    }
  ...
  fetch('/api/places')
    ...
    .then(data => {
      ...
      if (placeId) {
        ...
        if (place) {
          setTimeout(() => showPlaceDetails(place), 300);
        }
      }
    })
    .catch(err => {
